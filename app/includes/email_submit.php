...
//to chech if the message field is empty or not
	if (empty($emailUsers)) {
		...
		echo "<script>
        alert('$message')
        window.location.replace('http://japanesestudyresources.epizy.com/app/app.php?#news-letter');
    	</script>";
...
//to check if the email address is valid or not
        ...
        if($resultCheck > 0) {
			...
			window.location.replace('http://japanesestudyresources.epizy.com/app/app.php?');
			</script>";
            exit();
        }
        else {
